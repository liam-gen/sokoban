...
void gestion_deplacement_caisse(t_plateau plateau, char *positionSuivante, char *positionApresSuivante, int *deplacements, int playerX, int playerY);
    ...
    while(!isFinished)
    {
        if (kbhit()){
            ...
            if(gagne(plateau)){
                ...
                printf("Nombre de déplacements pour finir ce niveau : %d\n", deplacements);
                isFinished = true;
            }
        }
    }
...
* @param filename de type chaîne de caractères, Entrée : nom du fichier de niveau.
...
* @param plateau de type t_plateau, Entrée : tableau du plateau.
* @param nomDuFichier de type chaîne de caractères, Entrée/Sortie : nom du fichier de niveau.
*
*/
void demarrer_partie(t_plateau plateau, char nomDuFichier[30])
{
...
void position_joueur(t_plateau plateau, int *posX, int *posY)
{
    for (int ligne = 0 ; ligne < TAILLE ; ligne++){
        for (int colonne = 0 ; colonne < TAILLE ; colonne++){
            if(plateau[ligne][colonne] == PLAYER || plateau[ligne][colonne] == PLAYER_SUR_CIBLE)
            {
                (*posX) = ligne;
                (*posY) = colonne;
...
bool gagne(t_plateau plateau)
{
    ...
    for (int ligne = 0 ; ligne < TAILLE ; ligne++){
        for (int colonne = 0 ; colonne < TAILLE ; colonne++){
            // Si il y a encore une cible ou qu'un joueur est sur une cible on a pas encore gagné
            if(plateau[ligne][colonne] == CIBLE || plateau[ligne][colonne] == PLAYER_SUR_CIBLE) aGagner = false;
        }
    }
...
* @param positionSuivante de type t_plateau, Entrée : position après le joueur (celle que le joueur va prendre)
...
void gestion_deplacement_caisse(t_plateau plateau, char *positionSuivante, char *positionApresSuivante, int *deplacements, int playerX, int playerY){
    ...
    // Déplacer la caisse de une case et si c'est une cible on met une caisse sur une cible
    ...
    // Déplacer le joueur et si la position suivante est une cible ou une caisse sur une cible, on met le joueur sur une cible sinon on met juste un joueur
...
void deplacer(t_plateau plateau, int posX, int posY, int *deplacements)
{
    ...
    // Si on est pas contre un bord
    if(*positionSuivante != MUR)
    {
        ...
        if(*positionSuivante != CAISSE && *positionSuivante != CAISSE_SUR_CIBLE)
        {
            ...
            // Si la position suivante est une cible, on ajoute un joueur sur la cible sinon on met juste un joueur
        ...
        // Sinon on tente de pousser une caisse mais on vérifie que la case derrière n'est pas un mur ou une autre caisse
        else if(
            *positionApresSuivante != MUR && *positionApresSuivante != CAISSE && *positionApresSuivante != CAISSE_SUR_CIBLE
        )
        {
            gestion_deplacement_caisse(plateau, positionSuivante, positionApresSuivante, deplacements, playerX, playerY);
        }
    }
}
